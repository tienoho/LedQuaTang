  var Product = function () {
    return {
        ...
        loadData: function (pageIndex) {
            var self = this;
            let satatus = $('#satatus').val();
            var keyWord = $('#txtSearch').val();
            $("#loading").show();
            AjaxService.POST("/Admin/Product/ListData", { satatus: satatus, keyWord: keyWord, pageIndex: pageIndex }, function (res) {
                let texttitle = "Danh sách " + res.totalModel + " sản phẩm";
                $("#titleTable").text(texttitle);
        ...
        loadfrmAdd: function () {
            modal.Render("/Admin/Product/Add", "Thêm mới menu", "modal-lg");
        },
        handleAddImg: function (id) {
            modal.Render("/Admin/Product/AddImg/" + id, "Quản lý nhiều ảnh", "modal-lg");
        },
        loadImgMore: function (id) {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $.each(data,function (i,item) {
                        html += `<div id="img_${i}" style="margin-bottom:2px">
                                      <img style="width:100px;" src="${item}"/>
                                      <a href="javascript:void(0)" data-img="${item}" style="color:red" onclick="Product.deleteImgeMore(${i},'${item}')">
        ...
        checkCode: function(code) {
            AjaxService.GET("/Admin/Product/CheckProductCode", { code: code }, function (res) {
                let add = $('#acctionadd').val();
                if (res.IsSuccess == false) {
                    if (add == '1') {
                        document.getElementById("btnsaveadd").disabled = true;
                        document.getElementById("btnsavecontinue").disabled = true;
                    } else {
                        document.getElementById("btnsaveadd").disabled = true;
                    }
                    $('#product_code').text(res.Messenger);
        ...
        deleteImgeMore: function (i,imgSrc) {
            $('#img_' + i).remove();
        ...
        loadfrmDetail: function (id) {
            modal.Render("/Admin/Product/Detail/" + id, "Chi tiết sản phẩm", "modal-lg");
        },
        ...
        active: function (id) {
            ...
            swal({
                ...
                if (isConfirm) {
                    AjaxService.POST("/Admin/Product/ChangeStatus", { id: id }, function (res) {
                        Product.loadData(1);
                        alertmsg.success(res.Messenger);
                    });
                }
        ...
        onmultidelete: function () {
            ...
            if ($("table tbody").find("input[type=checkbox]:checked").length == 0) {
                ...
                swal({
                    ...
                    if (isConfirm) {
                        AjaxService.POST("/Admin/Product/DeleteAll", { lstid: $("#hdfID").val() }, function (res) {
                            self.pageIndex = 1;
        ...
        onupdateposittion: function () {
            ...
            AjaxService.POST("/Admin/Product/UpdatePosition", { value: strValue }, function (res) {
                if (res.IsSuccess == true) {
                    alertmsg.success(res.Messenger);
                    $("#gridData").html(res.ViewContent);
                    displayProduct();
